...
*
*  Autores: Michel Gomes de Andrade
...
unsigned int findFreeBlock(Disk *disk){
	...
	for (int i = freeSpaceSector; i < freeSpaceSector + freeSpaceSize; i++){	// Itera sobre os setores de espaço livre.
		if (diskReadSector(disk, i, buffer) == -1) // Lê um setor de espaço livre. Retorna -1 em caso de falha.
			return -1;

		for (int j = 0; j < DISK_SECTORDATASIZE; j++){			 // Itera sobre os bytes do setor de espaço livre.
			int freeBit = firstZeroBit(buffer[j]); // Encontra o primeiro bit livre no byte.

			if (freeBit != -1){ // Verifica se um bit livre foi encontrado.
...
int myFSWrite (int fd, const char *buf, unsigned int nbytes) {
	...
	while (bytesWritten < nbytes){ // Loop principal para escrever dados
		...
		for (int i = firstSector; i < sectorsPreBlock && bytesWritten < nbytes; i++){ // Loop para escrever dados em cada setor dentro do bloco real.
			if (diskReadSector(file->disk, currentBlock + i, diskBuffer) == -1)
				return -1; // Lê o setor atual do disco para o buffer.

			for (int j = firstByteInSector; j < DISK_SECTORDATASIZE && bytesWritten < nbytes; j++){ // Loop para copiar dados do buffer de entrada (buf) para o buffer do disco.
	...
	if (file->lastByteRead >= fileSize){			 // Se o último byte lido for maior ou igual ao tamanho total do arquivo
...
/*int myFSWrite(int fd, const char *buf, unsigned int nbytes) {
    ...
    File *file = files[fd]; // Ponteiro para o arquivo
    if (file == NULL) {
        return -1; // Arquivo não encontrado
    }

    unsigned int fileSize = inodeGetFileSize(file->inode); // Tamanho atual do arquivo
    unsigned int bytesWritten = 0; // Contador de bytes escritos
    unsigned int currentInodeBlockNum = file->lastByteRead / file->blocksize; // Número do bloco atual no inode
    unsigned int offset = file->lastByteRead % file->blocksize; // Deslocamento dentro do bloco
    unsigned int currentBlock = inodeGetBlockAddr(file->inode, currentInodeBlockNum); // Bloco atual no disco
    unsigned char diskBuffer[DISK_SECTORDATASIZE]; // Buffer para leitura/escrita de setores

    while (bytesWritten < nbytes) { // Loop principal para escrever os dados
        unsigned int sectorsPerBlock = file->blocksize / DISK_SECTORDATASIZE; // Número de setores por bloco
        unsigned int firstSector = offset / DISK_SECTORDATASIZE; // Primeiro setor dentro do bloco
        unsigned int firstByteInSector = offset % DISK_SECTORDATASIZE; // Deslocamento dentro do setor

        // Se o bloco atual for 0, aloca um novo bloco
        if (currentBlock == 0) {
            currentBlock = findFreeBlock(file->disk); // Tenta alocar um novo bloco
            if (currentBlock == -1) {
                break; // Falha ao alocar um novo bloco
            }

            // Adiciona o novo bloco ao inode
            if (inodeAddBlock(file->inode, currentBlock) == -1) {
                setBlockFree(file->disk, currentBlock); // Libera o bloco se falhar em adicioná-lo ao inode
                break;
            }
        }

        // Escreve os dados em cada setor dentro do bloco
        for (int i = firstSector; i < sectorsPerBlock && bytesWritten < nbytes; i++) {
            // Lê o setor atual do disco para o buffer
            if (diskReadSector(file->disk, currentBlock + i, diskBuffer) == -1) {
                return -1; // Falha na leitura do setor
            }

            // Copia os dados do buffer de entrada (buf) para o buffer do disco
            for (int j = firstByteInSector; j < DISK_SECTORDATASIZE && bytesWritten < nbytes; j++) {
                diskBuffer[j] = buf[bytesWritten];
                bytesWritten++;
            }

            // Escreve o setor atualizado de volta no disco
            if (diskWriteSector(file->disk, currentBlock + i, diskBuffer) == -1) {
                return -1; // Falha na escrita do setor
            }

            firstByteInSector = 0; // Reseta o deslocamento dentro do setor após o primeiro setor
        }

        offset = 0; // Reseta o deslocamento dentro do bloco
        currentInodeBlockNum++; // Avança para o próximo bloco do inode
        currentBlock = inodeGetBlockAddr(file->inode, currentInodeBlockNum); // Obtém o próximo bloco no disco
    }
    ...
    if (file->lastByteRead > fileSize) {
        inodeSetFileSize(file->inode, file->lastByteRead); // Atualiza o tamanho do arquivo
        inodeSave(file->inode); // Salva o inode no disco
    }
...
int installMyFS (void) {
    ...
    if (fsInfo->fsid == -1) {
        free(fsInfo);
        return -1; // Falha no registro do sistema de arquivos
    }
